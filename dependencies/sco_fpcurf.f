      subroutine sco_fpcurf(iopt,x,y,w,m,xb,xe,k,s,nest,tol,maxit,k1,k2,
...
c  evaluate the (k+1) non-zero b-splines at xi and store them in q.
  90      call sco_fpbspl(t,n,k,xi,l,h)
          do 95 i=1,k1
            q(it,i) = h(i)
            h(i) = h(i)*wi
  95      continue
...
c  calculate the parameters of the givens transformation.
            call sco_fpgivs(piv,a(j,1),cos,sin)
c  transformations to right hand side.
            call sco_fprota(cos,sin,yi,z(j))
...
c  transformations to left hand side.
              call sco_fprota(cos,sin,h(i1),a(j,i2))
 100        continue
 110      continue
...
c  backward substitution to obtain the b-spline coefficients.
        call sco_fpback(a,z,nk1,k1,c,nest)
...
c  t(j+k) <= x(i) <= t(j+k+1) and store it in fpint(j),j=1,2,...nrint.
 ...
 180    continue
        ...
        do 190 l=1,nplus
c  add a new knot.
          call sco_fpknot(x,m,t,n,fpint,nrdata,nrint,nest,1)
...
c  the row of matrix b is rotated into triangle by givens transformation
 ...
 270      continue
          ...
          do 290 j=it,nk1
            piv = h(1)
c  calculate the parameters of the givens transformation.
            call sco_fpgivs(piv,g(j,1),cos,sin)
c  transformations to right hand side.
            call sco_fprota(cos,sin,yi,c(j))
...
c  transformations to left hand side.
              i1 = i+1
              call sco_fprota(cos,sin,h(i1),g(j,i1))
              h(i) = h(i1)
 280        continue
            h(i2+1) = 0.
 290      continue
 300    continue
c  backward substitution to obtain the b-spline coefficients.
        call sco_fpback(g,c,nk1,k2,c,nest)
...
c  find the new value for p.
        p = sco_fprati(p1,f1,p2,f2,p3,f3)
 360  continue
