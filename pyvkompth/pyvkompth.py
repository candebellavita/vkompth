import os, sys
import pyvkompthbb, pyvkompthdk
import numpy as np
...
def update():
    ...
    plt.suptitle(r'QPO={:.1f} Hz'.format(qpo_freq)+'\n'+r'L={:.2f}km  $kT_e$={:.2f}keV  $kT_s$={:.2f}keV  $\tau$={:.2f}  $\eta$={:.2f}  $a_f$={:.1f}km'.format(size, kTe, kTs, tau, eta, af), y=0.99)
